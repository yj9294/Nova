...
//  OceanBrowser
//
//  Created by yangjian on 2023/4/21.
//
...
class GADUtil: NSObject {
    ...
    // 本地记录 配置
    fileprivate var adConfig: ADConfig? {
        set{
...
extension InterstitialADModel {
    ...
    override func present(from vc: UIViewController? = nil) {
        let vc = UIApplication.shared.keyWindow?.rootViewController ?? UIViewController()
        interstitialAd?.present(fromRootViewController: vc.presentVC)
    }
